...
int main(void)
{
	...
	for (;;)
    {
        ...
        // Next line: Sawtooth test signal
        //next_sample++;
        // Get audio samples from free-running ADC, shift it to make it visible
        next_sample = ADC_GetResult() << 5;
        // Spin loop until the bus is clear, keep on chucking them USB packets
        while (!Audio_Device_IsReadyForNextSample(&Microphone_Audio_Interface)){
            Audio_Device_USBTask(&Microphone_Audio_Interface);
            USB_USBTask();
        };
        Audio_Device_WriteSample16(&Microphone_Audio_Interface, next_sample);
	}
}
